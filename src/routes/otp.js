...
const { sendOtpStart, checkOtpCode } = require("../lib/twilio"); // tes helpers fournis
const router = Router();

// Petite "mémoire" en RAM pour marquer un phone comme vérifié pendant X min
const otpVerified = new Map(); // phone -> { purpose, exp:number }

// 5 minutes de validité après vérification du code
...
// === POST /api/auth/otp/start  { phone, purpose } ===
router.post("/otp/start", async (req, res) => {
  try {
    ...
    return res.status(500).json({ error: "Envoi OTP impossible" });
  }
});
...
router.post("/otp/verify", async (req, res) => {
  try {
    ...
    return res.json({ ok: true, message: "Code validé" });
  } catch (e) {
    console.error("otp/verify error:", e);
    return res.status(500).json({ error: "Vérification OTP impossible" });
  }
});

// === POST /api/auth/password/reset  { phone, new_password } ===
router.post("/password/reset", async (req, res) => {
  try {
    ...
    const [result] = await pool.query(
      "UPDATE users SET password_hash = ? WHERE phone = ? LIMIT 1",
      [hash, phone]
    );
