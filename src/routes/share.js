...
function absUrl(req, maybeUrl, { apiBase, webBase } = {}) {
  ...
  const WEB = webBase || env.FRONT_WEB_BASE_URL || process.env.FRONT_WEB_BASE_URL || originFromReq;
  const API = apiBase || env.API_PUBLIC_ORIGIN || process.env.API_PUBLIC_ORIGIN || originFromReq;

  // /uploads => API publique, sinon web
...
router.get("/product/:id", async (req, res, next) => {
  ...
  try {
    ...
    // ✅ Produit + image principale (product_images sinon cover)
    ...
    const title = escapeHtml(
      `${p.name || "Produit Duumini"}${p.shop_name ? ` — ${p.shop_name}` : ""}`
    );

    const descRaw =
      p.description || "Découvrez ce produit africain disponible sur Duumini.";
    const shortDesc = descRaw.length > 180 ? descRaw.slice(0, 177) + "..." : descRaw;
    const description = escapeHtml(shortDesc);

    // ✅ Image OG (priorité: product_images -> p.cover -> shop_cover -> shop_logo -> fallback)
    ...
    // ✅ URL canonique (la page front)
    // ton front : /products/:id (ou slug si tu veux)
    const finalUrl = `${webBase}/products/${encodeURIComponent(p.id)}`;

    // ✅ URL de partage (celle qui contient les OG tags)
...
<html lang="fr">
  <head>
    ...
    <!-- Pas d'indexation + limiter caches bots -->
    <meta name="robots" content="noindex, nofollow, noarchive" />
    <link rel="canonical" href="${finalUrl}" />

    <!-- Open Graph (Meta / WhatsApp / etc.) -->
