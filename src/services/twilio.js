...
async function sendOtpStart(phone, purpose = "signup") {
  ...
  const res = await client.verify.v2.services(env.TWILIO_VERIFY_SID).verifications.create({ to: phone, channel: "sms", locale: "fr" });
...
async function checkOtpCode(phone, code) {
  ...
  if (!client || !env.TWILIO_VERIFY_SID) throw new Error("Twilio not configured");
  const res = await client.verify.v2.services(env.TWILIO_VERIFY_SID).verificationChecks.create({ to: phone, code });
